...
@todo: This program still under check, there are possible some bugs in it.
"""
class VariationalBayes(object):
    ...
    def _initialize(self, data, truncation_level=5, alpha=1., sigma_a=1., sigma_x=1.):
        ...
        if self._finite_mode:
            ...
            #print self._alpha, self._tau.shape
            #print numpy.min(1., self._alpha), numpy.random.random(self._tau.shape) - 0.5;
    ...
    """
    compute the variational evidence lower bound (i.e. log_likelihood) in log scale
    """
    ...
    def export_snapshot(self, directory):
        numpy.savetxt(os.path.join(directory, "A-matrix-%i" % self._counter), self._phi_mean);
        #numpy.savetxt(os.path.join(directory, "X-matrix-%i" % self._counter), self._X);
        numpy.savetxt(os.path.join(directory, "Z-matrix-%i" % self._counter), self._nu);
