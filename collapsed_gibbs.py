...
class CollapsedGibbs(MonteCarlo):
    """
    ...
    def _initialize(self, data, alpha=1.0, sigma_a=1.0, sigma_x=1.0, initial_Z=None, initial_A=None, A_prior=None):
        super(CollapsedGibbs, self)._initialize(data, alpha, sigma_a, sigma_x, initial_Z, initial_A, A_prior);
    ...
    """
    sample the dataset to train the parameters
    """
    def learning(self):
        #, iteration, directory="../../output/tmp-output/"
        #import os, shutil
        ...
        for (object_counter, object_index) in enumerate(order):
            if object_counter > 0 and object_counter % 1000 == 0:
        ...
        #print("K: %i\tlikelihood: %f" % (self._K, self.log_likelihood_model()));
    ...
    def sample_Zn(self, object_index, M_i, log_det_M_i):
        ...
        for (feature_counter, feature_index) in enumerate(order):
            if feature_index in non_singleton_features:
                ...
                if numpy.random.random() < Znk_is_0:
                    self._Z[object_index, feature_index] = 0;
                    self._M = M_tmp_0;
    ...
    def metropolis_hastings_K_new(self, object_index, singleton_features, M_i, log_det_M_i):
        ...
        if numpy.random.random() < accept_new:
            self._Z = Z_new;
            self._K = K_new;
